...
public class TextParser implements AirlineParser<Airline> {
    ...
    public ArrayList<Airline> parseAll() throws ParserException {
        try (
            ...
            while ((textFileString = br.readLine()) != null && textFileString.length() != 0) {
                ...
                // Making sure there are correct number of arguments
                String[] listOfWords = textFileString.split("\\|");
                if(listOfWords.length != 8) {
                    throw new ParserException("Could not parse response.");
                }
                ...
                if (textFileString != null) {
                    ...
                    Optional<Airline> optionalAirline = airlines.stream().filter(a ->  a.getName().equals(airline.getName())).findFirst();

                    if(optionalAirline.isEmpty()) {
                        airlines.add(airline);
                    } else {
                        optionalAirline.get().addFlight(flight);
                    }
                }
            }
            return airlines;
